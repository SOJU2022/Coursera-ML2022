...
def sigmoidGradient(z):
    ...
    g = sigmoidFunc(z) * (1 - sigmoidFunc(z))
...
def checkNNGradients(nnCostFunction, lambda_=0):
    ...
    costFunc = lambda p: nnCostFunction(p, X, y, input_layer_size, hidden_layer_size,
                                        num_labels, lambda_)
...
def predict(Theta1, Theta2, X):
    ...
    p = np.zeros(m)
    h1 = sigmoidFunc(np.dot(np.concatenate([np.ones((m, 1)), X], axis=1), Theta1.T))
    h2 = sigmoidFunc(np.dot(np.concatenate([np.ones((m, 1)), h1], axis=1), Theta2.T))
